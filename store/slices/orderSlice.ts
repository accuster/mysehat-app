...
export const createOrder = createAsyncThunk(
  ...
  async (payload: CreateOrderPayload, { rejectWithValue }) => {
    try {
      ...
      console.log('User ID:', payload.user_id || 'NOT SET - will be set in SelectUser');
...
const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(updateOrderUser.fulfilled, (state, action: PayloadAction<{ orderId: string; userId: string }>) => {
